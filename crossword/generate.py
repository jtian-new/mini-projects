...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        """
        revised = False
        condition = self.crossword.overlaps[(x,y)]
        if condition is None:
            return revised
        for x_word in self.domains[x].copy():
            arc_consistency = False
            for y_word in self.domains[y]:
                if x_word[condition[0]] == y_word[condition[1]]:
    ...
    def consistent(self, assignment):
        ...
        for x in assignment.keys():
            ...
            if assignment[x] in values:
                return False
            else:
                values.add(assignment[x])

            x_neighbors = self.crossword.neighbors(x)
            for y in x_neighbors.intersection(set(assignment.keys())):
                condition = self.crossword.overlaps[(x,y)]
                if condition is not None and assignment[x][condition[0]] != assignment[y][condition[1]]:
                    return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        for x_val in self.domains[var]:
            ...
            for y in var_neighbors:
                ...
                for y_val in self.domains[y] - set(assignment.values()):
                    if x_val == y_val:
                        count += 1
                        continue
                    if overlap is not None:
                        if x_val[overlap[0]] != y_val[overlap[1]]:
                            count += 1
                            continue
            rule_out.append((x_val, count, len(var_neighbors)))
            
        def takeSecond(elem):
            return elem[1]
            
        rule_out.sort(key=takeSecond)

        return [i for (i,j) in rule_out]
    ...
    def select_unassigned_variable(self, assignment):
        ...
        for var in self.crossword.variables:
            if var not in assignment.keys():
                ...
                unassigned_var.append((var, domain_size, num_neighbors))
                
        def takeSecondThird(elem):
            return (elem[1],-elem[2])
        unassigned_var.sort(key=takeSecondThird)
